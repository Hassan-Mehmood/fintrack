...
export function InvestmentPortfoliosPanel({
  domain,
  holdings,
  portfolios,
}: InvestmentPortfoliosPanelProps) {
    ...
    mutationFn: async (values: PortfolioEditorValues) => {
      const payload = {
        ...
        cashAllocations:
          domain === "CRYPTO"
            ? []
            : Object.entries(values.cashAllocations)
    ...
    return (
      <Empty className="border">
        <EmptyHeader>
          ...
          <EmptyDescription>
            {domain === "CRYPTO"
              ? "Create a portfolio from individual crypto and stablecoin holdings."
              : "Create a portfolio from individual holdings and optionally allocate account cash."}
          </EmptyDescription>
        </EmptyHeader>
        <EmptyContent>
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <CardDescription>
                    ...
                    {domain === "SECURITIES"
                      ? ` · ${portfolio.cashAllocations.length} cash allocation${portfolio.cashAllocations.length === 1 ? "" : "s"}`
                      : ""}
                  </CardDescription>
                </div>
                <div className="flex gap-1">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      if (
                        window.confirm(
                          `Delete “${portfolio.name}”? Holdings, accounts, and transactions will remain intact.`,
                        )
                      )
              ...
              <div>
                ...
                <p className="font-mono font-medium">
                  {formatAmount(
                    ...
                    portfolio.metrics.baseCurrency,
                  )}
                </p>
              </div>
              <div>
...
type PortfolioEditorValues = {
  name: string;
  description: string;
  holdings: string[];
  cashAllocations: Record<string, string>;
};
...
function PortfolioEditor({
  ...
  const [selected, setSelected] = useState<string[]>(
    portfolio?.holdings.map(({ accountId, assetId }) =>
      positionKey(accountId, assetId),
    ) ?? [],
  );
  const [cash, setCash] = useState<Record<string, string>>(
    Object.fromEntries(
      portfolio?.cashAllocations.map(({ accountId, percentage }) => [
        accountId,
        percentage,
      ]) ?? [],
    ),
  ...
  const allocatedElsewhere = useMemo(() => {
    ...
    portfolios
      .filter(({ id }) => id !== portfolio?.id)
      .forEach((item) =>
        item.cashAllocations.forEach(({ accountId, percentage }) =>
          result.set(
            accountId,
            (result.get(accountId) ?? 0) + Number(percentage),
          ),
        ),
      );
    return result;
    ...
    >
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            {domain === "CRYPTO"
              ? "Select account-specific crypto and stablecoin positions."
              : "Select account-specific positions and allocate an optional percentage of each account’s current cash."}
          </DialogDescription>
        </DialogHeader>
          ...
          <section className="grid gap-2">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                A position can belong to only one portfolio.
              </p>
            </div>
            {active.length ? (
              active.map((holding) => {
                const key = positionKey(holding.accountId, holding.assetId);
                const belongsElsewhere = holding.portfolios.some(
                  ({ id }) => id !== portfolio?.id,
                );
                return (
                  <Label
              ...
              <p className="text-sm text-muted-foreground">
                Add an active holding before creating position membership.
              </p>
            )}
          </section>
          {domain === "SECURITIES" ? (
            <section className="grid gap-3">
              <div>
                <h3 className="text-sm font-medium">Fiat cash allocations</h3>
                <p className="text-xs text-muted-foreground">
                  Percentages are applied dynamically to each account’s current
                  cash balance.
                </p>
              </div>
              {investmentAccounts.map((account) => {
                ...
                return (
                  <div
                    key={account.id}
                    className="grid grid-cols-[1fr_8rem] items-end gap-3"
                  >
                    <div>
                    ...
                    <div className="grid gap-1">
                      <Label htmlFor={`cash-${account.id}`}>Percent</Label>
                      <Input
                        id={`cash-${account.id}`}
                        type="number"
                        min="0"
        ...
        <DialogFooter>
          ...
          <Button
            disabled={
              pending ||
              !name.trim() ||
              Object.entries(cash).some(
                ([accountId, value]) =>
                  Number(value) < 0 ||
                  Number(value) >
                    Math.max(0, 100 - (allocatedElsewhere.get(accountId) ?? 0)),
              )
            }
            onClick={() =>
              onSave({
