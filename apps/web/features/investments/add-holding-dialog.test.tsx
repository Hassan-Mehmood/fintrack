...
vi.mock("@/features/accounts/accounts-api", () => ({
  accountListQueryKey: (scope?: string) => ["accounts", scope ?? "ALL"],
  listAccounts: vi.fn().mockResolvedValue([]),
}));
vi.mock("@/features/portfolios/portfolios-api", () => ({
...
vi.mock("@/features/settings/settings-api", () => ({
  ...
  getSettings: vi
    .fn()
    .mockResolvedValue({ baseCurrency: "USD", exchangeRate: "275" }),
}));
vi.mock("@/features/assets/assets-api", () => ({
  ...
  getAssetMetadata: vi
    ...
    .mockResolvedValue({
      categories: [
        { id: "00000000-0000-4000-8000-000000000001", name: "Stock", order: 1 },
      ],
      riskProfiles: [],
    }),
  searchMarketAssets: vi
    .fn()
    .mockResolvedValue([
      {
        name: "Apple Inc.",
...
describe("AddHoldingDialog", () => {
  ...
  it("offers a dedicated stablecoin opening-balance flow", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Add stablecoin balance" }),
    ).toBeInTheDocument();
  ...
  it("keeps the user in the wizard for a provider-backed asset", async () => {
    renderDialog();
    fireEvent.change(screen.getByPlaceholderText("Search name or symbol"), {
      target: { value: "Apple" },
    });
    fireEvent.click(
  ...
  it("starts an existing crypto portfolio with a name and base currency", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Set up existing portfolio" }),
    ).toBeInTheDocument();
    fireEvent.change(screen.getByPlaceholderText("Search name or symbol"), {
      target: { value: "Apple" },
    });
    ...
    expect(
      screen.getByText(/Average purchase price \(optional/i),
    ).toBeInTheDocument();
  });
});
